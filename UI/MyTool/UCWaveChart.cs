    ...
    public class UCWaveChart : UCControlBase
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (m_currentSource == null || m_currentSource.Count <= 0)
            {
                for (int i = 0; i <= 4; i++)
                {
                    string strText = (40 / 4 * i).ToString();
            ...
            g.DrawPath(new Pen(new SolidBrush(m_lineColor), 3), path1);
        ...
        private List<KeyValuePair<string, double>> GetCurrentList()
        {
            ...
            else if (m_dataSource.Count > m_waveCount)
            {
            ...
            if (lst.Count == 1)
                lst.Insert(0, new KeyValuePair<string, double>(1.ToString(), 0));
            return lst;
        }
